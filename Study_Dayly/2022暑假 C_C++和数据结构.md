...
2、设有四个元素`A、B、C、D` 顺序进栈，在进栈过程中可以出栈，出栈次序错误的排列是（ D ） 
...
/*
题意分析：
    题目的意思是 需要写一个算法, 求出数组中最长的和谐子序列的长度
    和谐 就意味着 最大值最小值差为 1
    本题中 对子序列的定义是：属于数组的内容, 可以在不动顺序的前提下 删除元素
    举例说明就是：1 3 2 2 5 2 3 7
    1 3 2 5 2 3     就可以是一个子序列, 删除了一个 2
    1 3 2 2 2 3 7   也是一个子序列, 删除了一个 5
    但是 不能这样 1 3 2 2 5 7 2 不能改变顺序, 只能删除

思路分析：
    和谐子序列中只可能存在两种元素, 第三种元素出现时 序列的和谐就被打破了
    而由于 序列删除元素就可以的到子序列, 所以可以通过删除得出只剩两种元素的子序列
    此两种元素可以只相差1, 就可以说 这两种相差1 的元素的最多时, 就是最长和谐子序列
    所以可以通过哈希表统计原序列元素 并遍历哈希表计算相差1的元素的数量
*/
...
public:
    int findLHS(vector<int>& nums) {
        ...
        int max_harsubs = 0;
        for(auto e : numsTable) {
            if(numsTable.find(e.first+1) != numsTable.end()) {
                int e1c = numsTable[e.first+1];
                if((e.second+e1c) > max_harsubs)
                {
                    max_harsubs = e.second + e1c;
                }
            }
        }
